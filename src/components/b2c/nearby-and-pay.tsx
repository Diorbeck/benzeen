...
export function NearbyAndPay({ locale, data }: { locale: string; data: HomeStations }) {
      ...
      <section className="rounded-card border border-gray-200 bg-white p-5 dark:border-navy-700 dark:bg-navy-900">
        <div className="flex items-baseline justify-between gap-3">
          ...
          {!expanded && stations.length > visible.length ? (
            <button
              type="button"
              onClick={expand}
              className="inline-flex items-center gap-1 text-caption font-semibold text-primary-600 hover:gap-1.5 dark:text-sky-300"
            >
              {t('console.nearbyAll')}
        ...
        {state === 'loading' && (
          <ul className="mt-4 space-y-3" aria-hidden>
            {[0, 1, 2].map((i) => (
              <li key={i} className="h-12 animate-pulse rounded-control bg-gray-100 dark:bg-navy-800" />
            ))}
          </ul>
        )}

        {state !== 'loading' && rows.length === 0 && (
          <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">{t('console.nearbyEmpty')}</p>
        )}
              ...
              return (
                <li key={s.id} className="flex items-center justify-between gap-3 py-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-navy dark:text-white">{s.name}</p>
                  ...
                  <div className="flex shrink-0 items-center gap-4">
                    ...
                    <span className="text-right text-sm font-bold tabular-nums text-navy dark:text-white">
                      {main?.priceUzs ? formatMoney(main.priceUzs, locale) : '—'}
        ...
        <ul className="mt-4 space-y-3">
          {[
            ...
            <li key={key} className="flex items-start gap-3">
              ...
              <span className="text-sm leading-relaxed text-gray-200">{t(`console.${key}`)}</span>
            </li>
          ))}
        </ul>
      </section>
