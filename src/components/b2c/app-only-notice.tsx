...
export function StationFuelingAppOnly({ locale }: { locale: string }) {
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <h1 className="mt-4 max-w-3xl font-display text-[34px] font-bold leading-[1.02] tracking-[-0.025em] text-navy dark:text-white sm:text-[46px]">
        {t('title')}
        <br />
        <span className="text-primary-600 dark:text-sky-300">{t('titleAccent')}</span>
      </h1>
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.85fr)]">
        <section className="rounded-card border border-gray-200 bg-white p-6 dark:border-navy-700 dark:bg-navy-900">
          <p className="text-sm leading-relaxed text-gray-600 dark:text-gray-300">{t('lede')}</p>
          <ul className="mt-6 space-y-4">
            {points.map(({ icon: Icon, key }) => (
              <li key={key} className="flex items-start gap-3">
                ...
                <span className="text-sm leading-relaxed text-gray-700 dark:text-gray-300">{t(key)}</span>
              </li>
            ))}
          </ul>
